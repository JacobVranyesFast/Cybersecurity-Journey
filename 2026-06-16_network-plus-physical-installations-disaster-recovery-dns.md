# Physical Installations, Disaster Recovery, DHCP, DNS & Time Protocols
...
**DORA process:**
1. **Discover**: client broadcasts from 0.0.0.0 to 255.255.255.255 (UDP 68 to 67)
2. **Offer**: DHCP server broadcasts offer (UDP 67 to 68)
3. **Request**: client broadcasts requesting specific offer (UDP 68 to 67)
4. **Acknowledge**: server confirms, client applies IP config (UDP 67 to 68)
...
- Forward: name to IP (uses A/AAAA records)
- Reverse: IP to name (uses PTR records, configured separately)
...
| NTS (Network Time Security) | N/A | ~10ms | Adds authentication to NTP via TLS key exchange |
| PTP (Precision Time Protocol) | N/A | Nanoseconds | Hardware-based, used in industrial/financial environments |
...
Practiced basic KQL queries against the PassiveDns table:
...
- MDF/IDF and hot aisle/cold aisle design exist for the same underlying reason: keep infrastructure reachable, cool, and organized at scale
- RTO/RPO/MTTR/MTBF are frequently confused but answer different questions: how long to restore, how much data is lost, how long to fix, how long between failures
- DORA (Discover, Offer, Request, Acknowledge) is the actual sequence behind every DHCP lease, and DHCP relay exists specifically because broadcasts don't cross routers
- SLAAC lets IPv6 devices self-assign addresses without a DHCP server, using NDP router advertisements instead
- DNSSEC, DoT, and DoH all address different gaps in DNS security: authenticity, transport encryption, and encryption over HTTPS respectively

**Big Picture:** DHCP, DNS, and time sync all run quietly in the background until they break, and knowing exactly how each one is supposed to work is what makes a misconfiguration or an attack against them obvious instead of invisible.
