# Week 2 Day 2 — Wireshark, tcpdump, nmap & Subnetting
...
Captured live traffic browsing to http://example.com.
Applied filters to isolate traffic at each OSI layer:

- **dns** — observed DNS query/response pair. Note: Brave 
  uses DNS over HTTPS by default — had to use nslookup 
  from Command Prompt to generate visible port 53 traffic
- **tcp** — observed full 3-way handshake (SYN, SYN-ACK, ACK)
- **http** — captured plaintext GET request and 200 OK 
  response — demonstrates why HTTP is insecure
- **arp** — observed Layer 2 MAC address resolution
- **icmp** — captured ping request/reply pairs

### SOC Relevance
Wireshark is used in SOC investigations to detect 
unencrypted traffic, unusual DNS queries, port scans, 
data exfiltration, and malware C2 traffic.

---

## tcpdump — Command Line Traffic Analysis (Kali VM)

Completed TryHackMe tcpdump room and ran hands-on 
captures on home lab.
...
Note: complex filter expressions require single quotes — 
double quotes cause syntax errors.

### TCP Flag Reference
[S]  = SYN      [S.] = SYN-ACK  [.]  = ACK
[P.] = PSH-ACK  [F.] = FIN      [R]  = RST

---

## nmap — Network Scanning (TryHackMe Room)

Completed TryHackMe nmap room. Scanned 10.67.137.11.

### Scan Types
| Type | Command | Sudo | Method |
|------|---------|------|--------|
| TCP Connect | -sT | No | Full 3-way handshake — slower, detectable |
...
Key: running nmap without sudo forces TCP Connect 
because unprivileged users can't send raw packets.

### Target Results — 10.67.137.11
22/tcp   open  ssh      OpenSSH 9.6p1 Ubuntu
8008/tcp open  http     lighttpd 1.4.74

4 additional open ports (echo, tcpwrapped, daytime, qotd)

Web server found on 8008 — accessed via browser to retrieve flag.

---

## Static IP Configuration

Fixed persistent DHCP issue on both lab VMs:
