# Network+ Domain 5 Troubleshooting Review & Raspberry Pi Remote Access

**Date:** June 19, 2026
**Platform:** Professor Messer N10-009 + Home Lab (Raspberry Pi, Tailscale, Apollo/Moonlight)
**Domain:** Network+ N10-009 Domain 5 | Home Lab Infrastructure
...
- Troubleshooting methodology has a defined step order for a reason: skipping steps is how you fix the symptom and miss the actual cause
- Layer 2 loop prevention (STP) and VLAN misassignment are still soft spots, going into Anki for daily review
- Building the Pi relay was the same triage process taught in Domain 5: work through the stack layer by layer (ping, then port, then firewall, then application) instead of guessing

**Big Picture:** The same methodology that diagnoses a bad cable or a misconfigured VLAN also diagnosed the Pi's connectivity issue tonight: isolate the layer, confirm what's working, narrow down what isn't.
