...
- **Conditional logic:** `iff()`, `case()` for inline row labeling.
- **Filtering:** `startswith`, `endswith`, `in`, `!in`, `has_any`.

## Query Patterns Practiced
...
Multiple named aggregations per group:
```kusto
Email
| summarize total_emails = count(), unique_recipients = dcount(recipient) by sender
```

Distinct IP tracking per user, filtered on the result:
```kusto
...
| summarize dcount(sender_domain)
```
...
- Unaliased aggregation columns get auto-generated names (`count_`, `dcount_src_ip`). Alias explicitly so downstream `where` clauses reference the right column.
