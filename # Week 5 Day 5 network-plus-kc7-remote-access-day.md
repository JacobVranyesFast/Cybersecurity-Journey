...
The Pi's IP drift is the same class of problem as a stale lease in production — the fix is a DHCP reservation, not just finding the new address. Setting that up next for the whole Pi fleet and Jimba-PC. The Apollo issue was a good reminder that "broke after a reboot" is its own diagnostic category — service execution context matters as much as config.
